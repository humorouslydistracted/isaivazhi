...
final class AlbumArtHelper {
    ...
    static File getArtCacheDir(Context ctx) {
        File dir = new File(ctx.getCacheDir(), "albumart");
        if (!dir.exists()) dir.mkdirs();
        return dir;
    }

    static String artCacheKey(String filePath) {
        return Integer.toHexString((ART_CACHE_KEY_PREFIX + filePath).hashCode());
    }

    static File cachedArtFile(Context ctx, String filePath) {
    ...
    static String extractAndCacheArt(Context ctx, String filePath) {
        try {
